## Install Latest Microsoft PowerShell to ensure compatibility with the latest features and security updates
...
Get-Service -Name SSH-Agent -ErrorAction SilentlyContinue | Set-Service -StartupType Disabled
...
#Get-Service -Name SSH-Agent -ErrorAction SilentlyContinue | Restart-Service

## Set the variable "SSH_AUTH_SOCK" for current session
if ([System.Environment]::GetEnvironmentVariable('SSH_AUTH_SOCK') -eq $null) 
  { 
    if ($Admin) 
